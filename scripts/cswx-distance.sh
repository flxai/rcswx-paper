...
#SBATCH --mem=500G
#SBATCH --output=logs/distance-cswx-slurm/%j.out
#SBATCH --error=logs/distance-cswx-slurm/%j.err
#
# SLURM wrapper for cswx-distance.py (one job per CSV)
#
# Example usage:
#   find jobs_lists_split -maxdepth 1 -type f -name 'compute_distance_cifar10_0_*' -print0 \
#   | xargs -0 -n1 -I{} sbatch --export=ALL,CSV="{}" scripts/cswx-distance.sh
...
srun --cpu-bind=cores \
  python -u scripts/cswx-distance.py "$CSV" \
    --workers "${NWORKERS}" \
