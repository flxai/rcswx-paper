...
# Run the script
python "$WD/../main.py" --config $CONFIG_FILE --device cuda:$GPU_ID &> $LOG_FILE
python "$WD/../test.py" --config $CONFIG_FILE --device cuda:$GPU_ID &>> $LOG_FILE
python "$WD/../plot.py" --config $CONFIG_FILE --device cuda:$GPU_ID
