...
from search_strategies.utils.constrained_smith_waterman import (
    cswx_distance,
)
...
def worker(a: str, b: str, results_dir: Path, logs_dir: Path, skip_existing: bool = True):
    ...
    with log_path.open("w", encoding="utf-8") as log:
        ...
        try:
            ...
            logp("Computing CSWX distance...")
            t0 = thread_time()
            dist = cswx_distance(dt_a, dt_b)
            td = thread_time() - t0
...
def main():
    ...
    ap.add_argument("--results-dir", type=Path, default=Path("results/distance-cswx"))
    ap.add_argument("--logs-dir", type=Path, default=Path("logs/distance-cswx"))
