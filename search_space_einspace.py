        ...
        return OrderedDict(
            {
                ...
                "other_shape": self.other_shape,
                "input_mode": self.input_mode,
                "other_mode": self.other_mode,
                "input_branching_factor": self.input_branching_factor,
                "output_branching_factor": self.output_branching_factor,
                "last_im_input_shape": self.last_im_input_shape,
...
class EinSpace:
    ...
    def get_available_options(self, search_state):
        """Get the possible operations that can be chosen, filtered by the current state."""
        # print(level, input_shape, other_shape, last_im_input_shape)
        if search_state.level == "terminal":
            return []
        options = deepcopy(self.available_options[search_state.level])
        ...
        options = self.filter_options(
            ...
            options,
            search_state.level,
            search_state.input_shape,
            search_state.other_shape,
            search_state.input_mode,
            search_state.input_branching_factor,
            search_state.module_depth,
        )
        return options
    ...
    def grow(self, search_state, operation):
        """
        Grow the current architecture with a chosen node.
        arguments:
            - search_state: the current state of the search
            - operation: the node to add to the architecture
        returns:
            - the new state of the search
        """
        if "im2col" in operation.__name__:
            search_state.last_im_input_shape = operation(
                **{"input_shape": search_state.input_shape}
            ).fold_output_shape
        # if we have chosen a non-terminal symbol
        # (i.e. sequential_module, branching_module or routing_module, computation_module),
        search_state.operation = operation
        search_state.node_id += 1
        if operation.__name__ == "sequential_module":
            search_state.module_depth += 1
            search_state.node_id
            # search_state.level = "first_fn"
        elif operation.__name__ == "branching_module":
            search_state.module_depth += 1
            # search_state.level = "branching_fn"
        elif operation.__name__ == "routing_module":
            search_state.module_depth += 1
            # search_state.level = "prerouting_fn"
        elif operation.__name__ == "computation_module":
            search_state.module_depth += 1
            # search_state.level = "computation_fn"
        else:
            try:
                # # update input information
                # search_state.input_shape = search_state.output_shape
                # search_state.input_mode = search_state.output_mode
                # search_state.input_branching_factor = search_state.output_branching_factor
                # update output information
                search_state.output_shape = self.recurse_shapes(
                    operation,
                    search_state.input_shape,
                    search_state.other_shape,
                    search_state.last_im_input_shape,
                    search_state.input_branching_factor,
                )
                search_state.output_mode = self.recurse_modes(
                    operation,
                    search_state.input_mode,
                    search_state.other_mode,
                )
                search_state.output_branching_factor = self.recurse_branching(
                    operation,
                    search_state.input_branching_factor,
                )
                # search_state.node_type = "terminal"
            except ArchitectureCompilationError as e:
                raise SearchSpaceSamplingError(
                    "Error when searching " + operation.__name__ + ": " + str(e)
                )
        return search_state
